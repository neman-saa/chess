...
import chess.domain.auth.{LoginInfo, NewPasswordInfo}
...
object validators {
  ...
  case class InvalidPassword(fieldName: String) extends ValidationFailure(s"password $fieldName is not a valid password")

  trait Validator[A] {
    def validate(a: A): ValidatedNel[ValidationFailure, A]
  }

  private def validateRequired[A](field: A, fieldName: String)(required: A => Boolean): ValidatedNel[ValidationFailure, A] =
  ...
  given userRegistrationValidator: Validator[UserRegistration] = (userRegistration: UserRegistration) => {
    val validNickName = validateRequired(userRegistration.nickname, "username")(nick => !nick.contains('-') && nick.length > 4 && nick.length < 15)
  ...
  given newPasswordValidator: Validator[NewPasswordInfo] = (newPasswordInfo: NewPasswordInfo) => {
    val validatePassword = validateRequired(newPasswordInfo.newPassword, "new password")(pass => pass.length > 5 && pass.length < 30)
