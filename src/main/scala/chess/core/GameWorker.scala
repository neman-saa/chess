...
class GameWorker[F[_] : Concurrent](game: Ref[F, GameInfo])(val id: UUID) {
  ...
  def availableMovesFromPrivate(gameInfo: GameInfo, from: Coordinate): F[List[Move]] = {
    ...
    (castle(gameInfo.turn, true) |+| castle(gameInfo.turn, false) |+| availableMovesFromWeak(gameInfo, from)).filter(move => isPositionLegal(makeMovePrediction(gameInfo, move))).pure[F]
  }
  ...
  private def availableMovesFromWeak(gameInfo: GameInfo, from: Coordinate): List[Move] = {
    ...
    def weakPawnMoves: List[Move] = {
        ...
        def enPassant: List[Move] = (turn, previousMove) match {
          case ("white", Some(SimpleMove (Figure.PAWN, ((from1, from2), (_, to2))))) if from1 == current._1 + 1 && from2 == current._2 + 2 && to2 == current._2 =>
            List (EnPassant ((current, (current._1 + 1, current._2 + 1) ) ) )
          case ("white", Some(SimpleMove (Figure.PAWN, ((from1, from2), (_, to2))))) if from1 == current._1 - 1 && from2 == current._2 + 2 && to2 == current._2 =>
            List (EnPassant ((current, (current._1 - 1, current._2 + 1) ) ) )
          case ("black", Some(SimpleMove (Figure.PAWN, ((from1, from2), (_, to2))))) if from1 == current._1 + 1 && from2 == current._2 - 2 && to2 == current._2=>
            List (EnPassant ((current, (current._1 + 1, current._2 - 1) ) ) )
          case ("black", Some(SimpleMove (Figure.PAWN, ((from1, from2), (_, to2))))) if from1 == current._1 - 1 && from2 == current._2 + - 2 && to2 == current._2=>
            List(EnPassant((current, (current._1 - 1, current._2 - 1))))
          case _ => Nil
          ...
          case "white" =>
            (current._2, board.board.filter(_.coordinate == (current._1, current._2 + 1))) match {
              case (2, Nil) if !board.board.exists(_.coordinate == (current._1, current._2 + 2)) =>
                List(SimpleMove(Figure.PAWN, (current, (current._1, current._2 + 1))), SimpleMove(Figure.PAWN, (current, (current._1, current._2 + 2))))
              case (7, Nil) =>
                List(Figure.KNIGHT, Figure.BISHOP, Figure.QUEEN, Figure.NoCastleROOK).map(figure => Promotion((current, (current._1, current._2 + 1)), figure))
              case (_, Nil) =>
        ...
        def take: List[Move] = turn match {
          case "white" if board.board.exists(figure => figure.coordinate == (current._1 + 1, current._2 + 1) && figure.color == "black") =>
  ...
  private def makeMovePrediction(gameInfo: GameInfo, move: Move): GameInfo = {
    ...
    val updatedBoard = move match {
      ...
      case SimpleMove(figure, fromTo) =>
        Board(FigureWithColor(figure, turn, fromTo._2) :: board.filter(figure => figure.coordinate != fromTo._1 && figure.coordinate != fromTo._2))

      case EnPassant(fromTo) => gameInfo.turn match {
        case "white" =>
          Board(FigureWithColor(Figure.PAWN, "white", fromTo._2) :: board.filter(_.coordinate != fromTo._1).filter(_.coordinate != (fromTo._2._1, fromTo._2._2 - 1)))
        case "black" =>
          Board(FigureWithColor(Figure.PAWN, "black", fromTo._2) :: board.filter(_.coordinate != fromTo._1).filter(_.coordinate != (fromTo._2._1, fromTo._2._2 + 1)))
      }

      case Castle(right, fromTo) => (turn, right) match {
        case ("white", true) =>
          Board(FigureWithColor(Figure.KING, "white", (7, 1)) :: FigureWithColor(Figure.ROOK2, "white", (6, 1)) :: board.filter(_.coordinate != (5, 1)).filter(_.coordinate != (8, 1)))
        case ("black", true) =>
          Board(FigureWithColor(Figure.KING, "black", (7, 8)) :: FigureWithColor(Figure.ROOK2, "black", (6, 8)) :: board.filter(_.coordinate != (5, 8)).filter(_.coordinate != (8, 8)))
        case ("white", false) =>
          Board(FigureWithColor(Figure.KING, "white", (3, 1)) :: FigureWithColor(Figure.ROOK2, "white", (4, 1)) :: board.filter(_.coordinate != (5, 1)).filter(_.coordinate != (1, 1)))
        case ("black", false) =>
          Board(FigureWithColor(Figure.KING, "black", (3, 8)) :: FigureWithColor(Figure.ROOK2, "black", (4, 8)) :: board.filter(_.coordinate != (5, 8)).filter(_.coordinate != (1, 8)))
        case (_, _) => Board(Nil)
      }
