...
class LiveGames[F[_]: Concurrent: Logger](xa: Transactor[F])(
  ...
  override def endGame(loser: UUID, id: RoomId): F[Unit] = for {
    ...
    _ <-
      players.traverse(players => sessions.offer(players._1, s"Game ended, $winner won.")
  ...
  override def connect(roomId: RoomId, player: UUID): F[Either[String, Unit]] = for {
    ...
    result <- mbRoom match {
      case Some(room) => room._1.length match {
        case 1 if room._1.contains(player) => startGame(roomId, (room._1.head, room._1.tail.head)).flatMap(_ => removeConnectingGame(roomId)).map(_ => Right({}))
        case 2 if room._1.contains(player) => playersAreConnecting.update(map => map + (roomId -> (room._1, 1))).map(_ => Right({}))
...
object LiveGames {
  ...
  def apply[F[_]: Concurrent: Logger](xa: Transactor[F])(sessions: Sessions[F], users: Users[F]): F[LiveGames[F]] = for {
