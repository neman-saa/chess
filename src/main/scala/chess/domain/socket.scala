...
object socket {
  ...
  object InputMessage {
    ...
    def parse(userId: UUID, text: String, roomId: UUID): InputMessage = text.split(";").toList match {
      ...
      case "simpleMove" :: figure :: x1 :: y1 :: x2 :: y2 :: Nil => MakeMove(SimpleMove(Figure.valueOf(figure), ((x1.toInt, y1.toInt), (x2.toInt, y2.toInt))), userId, roomId)
      case "enPassant" :: x1 :: y1 :: x2 :: y2 :: Nil => MakeMove(EnPassant(((x1.toInt, y1.toInt), (x2.toInt, y2.toInt))), userId, roomId)
      case "promotion" :: promotesTo :: x1 :: y1 :: x2 :: y2 :: Nil => MakeMove(Promotion(((x1.toInt, y1.toInt), (x2.toInt, y2.toInt)), Figure.valueOf(promotesTo)), userId, roomId)
      case "castle" :: right :: x1 :: y1 :: x2 :: y2 :: Nil => MakeMove(Castle(right.toBoolean, ((x1.toInt, y1.toInt), (x2.toInt, y2.toInt))), userId, roomId)
